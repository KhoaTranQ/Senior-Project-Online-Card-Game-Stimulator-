/// xf_insert(parent,index,node,...)
// Inserts up to 13 nodes at the desired position index of the given parent node.
// See: www.xmlfir.com/xf-insert
...
with (t__xml)
{
    ...
    if (argument_count < 3) {
        return xf_log(xf_insert, 1, "Missing argument count: " +  string(3 - argument_count) + ".");
    }
    ...
    if (!is_real(t_node) || !ds_exists(t_node, ds_type_list) || ds_list_size(t_node) != xf_prop_size) {
        return xf_log(xf_insert, 1, "argument0 is not a valid xml node.");
    }
    ...
    if (t_type != xf_type_element && t_type != xf_type_fragment) {
        return xf_log(xf_insert, 1, "argument0 does not support nested elements.");
    }
    ...
    while (t_i < t_arg_count)
    {
        ...
        if (!is_real(t_val) || !ds_exists(t_val, ds_type_list) || ds_list_size(t_val) != xf_prop_size) {
            return xf_log(xf_insert, 1, "argument" + string(t_i) + " is not a valid xml node.");
        }
        ...
        if (t_parent != noone && (t_val_type == xf_type_element || t_val_type == xf_type_fragment)) {
            ...
            // Check for error
            if (t_list == -1) {
                return xf_log(xf_insert, 1, "argument" + string(t_i) + " contains an invalid xml node.");
            }
            
            if (ds_list_find_index(t_list, t_node) > -1) {
                ds_list_destroy(t_list);
                return xf_log(xf_insert, 1, "Cannot make argument" + string(t_i) + " a child of itself.");
            }
    ...
    while (t_i < t_arg_count)
    {
        ...
        if (t_val_type != xf_type_fragment) 
        {
            ...
            if (ds_exists(t_parent, ds_type_list)) {
                ...
                if (ds_exists(t_children, ds_type_list)) {
                    ...
                    if (t_idx == -1) {
                        xf_log(xf_insert, 2, "argument" + string(t_i) + " has an invalid parent element.");
                    }
                    else {
                        // Detach from parent
            ...
            if (p_call_ev_relocate != noone && p_ev_relocate_on) {
                // script, event, node, prev parent, new parent, new index
                script_execute(p_call_ev_relocate, xf_ev_relocate, t_val, t_parent, t_node, t_pos);
            }
        ...
        while (t_idx < t_count2)
        {
            ...
            if (p_call_ev_relocate != noone && p_ev_relocate_on) {
                // script, event, node, prev parent, new parent, new index
                script_execute(p_call_ev_relocate, xf_ev_relocate, t_child, t_val, t_node, t_pos);
            }
