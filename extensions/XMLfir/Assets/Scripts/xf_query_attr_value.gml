/// xf_query_attr_value(list,name,op,value,...)
// Selects nodes in the given ds_list whose attribute value cmp equals true with at least one of the given value args.
// See: www.xmlfir.com/xf-query-attr-value
...
with (t__xml)
{
    ...
    if (argument_count < 4) {
        return xf_log(xf_query_attr_value, 1, "Missing argument count: " +  string(4 - argument_count) + ".");
    }
    ...
    if (!ds_exists(t_list, ds_type_list)) {
        return xf_log(xf_query_attr_value, 1, "argument0 is not a valid ds_list.");
    }
    
    // Get the attribute
    var t_name = string(argument[1]);
    
    // Get operator
    var t_op = string(argument[2]);
    ...
    switch (t_op)
    {
            ...
            while (t_i < argument_count)
            {
                if (!is_real(argument[t_i])) {
                    ...
                    if (string(real(argument[t_i])) != argument[t_i]) {
                        return xf_log(xf_query_attr_value, 1, "argument" + string(t_i) + " is not number.");
                    }
        ...
        default:
            return xf_log(xf_query_attr_value, 1, "argument1 is not a valid operator.");
            break;
    }
    ...
    switch (t_op)
    {
        ...
        case "=":
            while (t_i < t_count)
            {
                ...
                t_idx = ds_list_find_index(t_keys, t_name);
                ...
                t_vals  = ds_list_find_value(t_node, xf_prop_attrv);
                t_value = string(ds_list_find_value(t_vals, t_idx));
                
                if (ds_list_find_index(t_filter, t_value) == -1) {
                    ds_list_delete(t_list, t_i);
                    t_count--;
                    continue;
                }
        ...
        case ">":
            while (t_i < t_count)
            {
                ...
                t_vals  = ds_list_find_value(t_node, xf_prop_attrv);
                t_value = ds_list_find_value(t_vals, t_idx);
                t_found = false;
                t_j     = 0;
