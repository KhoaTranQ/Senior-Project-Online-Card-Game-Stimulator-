/// xf_get_next(node)
// Returns the id of the sibling positioned next to the given node.
// See: www.xmlfir.com/xf-get-next
...
with (t__xml)
{
    ...
    if (!is_real(argument0) || !ds_exists(argument0, ds_type_list) || ds_list_size(argument0) != xf_prop_size) {
        return xf_log(noone, xf_get_next, 1, "argument0 is not a valid xml node.");
    }
    ...
    if (t_parent == noone) {
        ...
        return xf_log(noone, xf_get_next, 2, "Parent node is not a valid xml structure.");
    }
    ...
    if (t_idx == -1) {
        return xf_log(noone, xf_get_next, 1, "Index not found.");
    }
    else if (t_idx + 1 == t_count) {
